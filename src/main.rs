...
use crate::types::{Arguments, ClickEvent, Monitor, UserStatus};
...
const SHOWN_STATUSES: [&str; 3] = ["800", "801", "802"];
...
fn i3blocks_format_monitor(
    ...
    for status in SHOWN_STATUSES {
        let user_count = *monitor.basic().get(status).expect("status not found in monitor").count();
        if user_count == 0 {
            continue;
        }

        let color = status_map.get(status).expect("status not in pulled data").color_hex();
...
fn read_stdin(tx: mpsc::Sender<ClickEvent>) {
    ...
    loop {
        ...
        tx.blocking_send(event).expect("channel closed by main thread");
    }
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let token: String = if let Some(token) = args.token {
        ...
        let mut buffer = String::new();
        let file = File::open(args.token_file.expect("neither token or token-file provided")).expect("unable to open file");
        let _ = io::BufReader::new(file).read_line(&mut buffer).expect("unable to read file");
        buffer.pop();
        buffer
    };
